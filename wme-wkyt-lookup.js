...
(function() {
    ...
    var debugLevel = 0;
    var kytcLookupVersion = "0.3";
    var kytcLookupChanges = "WME KYTC Lookup\nv" + kytcLookupVersion + "\nMove the mouse over a road and hit ALT+k to look it up.\n\nWhat's New\n------------------------------";
    kytcLookupChanges += '\n- To avoid accidental popups, you must press ALT+k instead of just the "k" key.';
    kytcLookupChanges += '\n- Added a "KYTC" link in the footer.  Click it to open the KYTC Functional Class map in the roughly the same location as the Waze map.';
    ...
    function init() {
        ...
        $('.WazeMapFooter').prepend(
            $('<div>').css({float:'right',height:'26px',paddingTop:'3px'}).append(
                $('<a>',{id:'kytc-button'}).text('KYTC').attr('href','javascript:void(0)').css({textDecoration:'none', color:'blue', fontWeight:'bold'}).click(kytcButton_click)
            )
        );
